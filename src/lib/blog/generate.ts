...
- タイトルは読者の興味を引く実務フック（例: 「〇〇補助金、今年度はココが変わる──〇〇事業者が今すぐ確認すべき3点」）
...
Markdown 書式を使う（note.com は見出し、箇条書き、太字、引用に対応）。
ただし **テーブル記法は note が描画しない**ので使わない。代わりに箇条書きで:
  - **対象**: xxx
  - **期限**: xxx
  - **金額**: xxx
のように太字ラベル + 項目の形に整える。

ハッシュタグは記事末尾に 3〜6個、半角シャープで付ける（例: #サーキュラーエコノミー #鹿児島 #脱炭素）。

出力は JSON で以下のスキーマに厳密に従う（コードブロックや説明なし、JSON のみ）:
{
  "title": "記事タイトル",
  "body": "Markdown 本文（ハッシュタグも含む）",
  "hashtags": ["#タグ1", "#タグ2", ...]
}`;
...
function parseJson(s: string): { title: string; body: string; hashtags?: string[] } {
  ...
  try {
    ...
    throw new Error('Could not parse blog post JSON from model output');
  }
}
